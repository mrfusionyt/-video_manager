/* ============================================================
   photos.js — JS страницы /photos
   • More (бесконечная загрузка) через /load_more_photos
   • пагинация (компактная, с ellipsis)
   • сортировка через data-sort (обрабатывается в base.js)
   Конфиг приходит через window.PHOTOS_CONFIG из шаблона.
   ============================================================ */
(function () {
    'use strict';

    var CFG = window.PHOTOS_CONFIG || {};
    ...
    /* -----------------------------------------------------------
       More — AJAX-подгрузка через /load_more_photos
       ----------------------------------------------------------- */
    if (moreBtn) {
        moreBtn.addEventListener('click', function () {
            ...
            var url = '/load_more_photos?page=' + nextPage
                    + '&sort=' + encodeURIComponent(currentSort)
                    + '&search=' + encodeURIComponent(currentSearch || '');
